...
class ImageFolder(data.Dataset):
    def __init__(self, root, split_dir='train', custom_classes=None,
                 base_size=64, transform=None, target_transform=None,mem=True):
        ...
        self.imgs = imgs
        if self.mem:
            self.mem_imgs = [Image.open(path).convert('RGB') for path, target in self.imgs]
    ...
    def __getitem__(self, index):
        ...
        img = self.mem_imgs[index] if self.mem else Image.open(path).convert('RGB') 
        imgs_list = get_imgs(img, self.imsize, transform=self.transform, normalize=self.norm)
        return imgs_list, target
...
class LSUNClass(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgs = get_imgs(img, self.imsize, transform=self.transform, normalize=self.norm)
        return imgs
...
class TextDataset(data.Dataset):
    ...
    def prepair_training_pairs(self, index):
        ...
        wrong_img = Image.open(wrong_img_name).convert('RGB')
        wrong_imgs = get_imgs(wrong_img, self.imsize, wrong_bbox, self.transform, normalize=self.norm)
